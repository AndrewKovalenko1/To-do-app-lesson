...
class Form extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      whatValue: "",
      whereValue: "",
      subjects:  [{name: "Business", color: "turquoise-color", value: 0},
  ...
  handleOnChangeWhat = event =>{
    this.setState({ "whatValue": event.target.value});
  }

  handleOnChangeWhere = event =>{
    this.setState({ "whereValue": event.target.value});
  }
...
fillingCheking =() => {
  let allIsFill = true;
    if((this.state.toggleOn ===false && this.state.timeValue ==="") || 
    this.state.dateValue==="" || this.state.iconValue==="plus" || this.state.whatValue==="" ||
    this.state.whereValue ===""){
      if(this.state.subject===null){this.setState({subject: this.state.subjects[0]})}
      allIsFill = false;
    }
  ...
  onTodayCounterUpdate = tasks => {
    ...
    const countFoToday = tasks.filter((task) => (this.DateStringToDate(task.date).getTime() == currDate.getTime())).length;
    ...
    onSummaryUpdate = tasks => {
      ...
      for (let subject of subjects) {
          subject.value = tasks.filter((task) => (task.subject === subject.name)).length;
      }
...
componentDidMount() {
  ApiService.getTasks().then(tasks => { 
    this.onTodayCounterUpdate((tasks.default===undefined)?tasks :tasks.default);
    ...
    return (
      ...
      <section>
        <div className={"flex-center main-column red-bcolor"}>
          ...
          <ButtonTo className="button1" id="button-to-page-add-task" text="VIEW TASKS" path="/" />
        </div>
        <div className={"flex-center main-column green-bcolor"}>
          ...
          <div className="flex-center main-section2">
            <IconDropdown handleOnClickIcon = {this.handleOnClickIcon} currentIcon = {this.state.iconValue}/>
            <form id="form-add-task">
              <SubjectDropdown subjects = {this.state.subjects}  handleOnClickSubject={this.handleOnClickSubject} subject= {this.state.subject}/>
              <input type="text" placeholder="What I have to do?*" value = {this.state.whatValue} onChange = {(event)=>this.handleOnChangeWhat(event)}/>
              <input type="text" placeholder="Where?*" value = {this.state.whereValue} onChange = {(event)=>this.handleOnChangeWhere(event)}/>
              <div className="date-container">
                <DatePicker value = {this.state.dateValue} 
                onChange = {(event)=>this.handleOnChangeDate(event)}/>
                {(this.state.toggleOn===false)? <TimePicker value = {this.state.timeValue} 
            ...
            <ButtonNewTask taskFill = {this.state.taskFill} onClick = {this.handleClickAddTask}/>
          </div>
        </div>
      </section>
    );
